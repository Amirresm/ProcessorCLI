...
public class DataPathDriver {
	...
	private void stageID() {
		...
		//setup control unit
		controlUnit.setup(oppCode, funcCode);

		//setup register file
		String writeRegisterMux = Mux.compute(regAddr2, regWAddr, true);                        //mux TODO: flag = REGDST
	...
	private void stageEXE() {
		//ALU
		String aluInput2 = Mux.compute(regReadData1, extendedOffset, true);                    //mux TODO: flag = ALUSRC
		aluResult = alu.compute(regReadData1, aluInput2, "get signal from control");         //TODO: fix alu signal
		...
		boolean isBranchTaken = AndUnit.and(alu.getZeroSignal(), true);                             //and TODO: flag = BRANCH
		nextPc = Mux.compute(nextPc, pcPlusOffset, isBranchTaken);                                  //mux TODO: flag =
		nextPc = Mux.compute(nextPc, jumpAddr, true);                                          //mux TODO: flag = JUMP
	}

	private void stageMEM() {
		mainMemory.setup(aluResult, true, true);                        //TODO: signals from control unit
		mainMemory.writeData(regReadData2);
		mainMemoryRead = mainMemory.readData();
	}

	private void stageWB() {
		String registerWriteData = Mux.compute(aluResult, mainMemoryRead, true);                //mux TODO: flag = MEMTOREG
